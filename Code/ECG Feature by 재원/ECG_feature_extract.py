...
from full_waveform_features import *


class Features:

    def __init__(self, file_path, fs, feature_groups):
    ...
    def extract_features(self, filter_bandwidth, n_signals=None, show=False,
                         normalize=True, template_before=0.2, template_after=0.4):
    ...
    def _get_file_names(self, n_signals):
        ...
        file_names = [file.split('.')[0] for file in sorted(os.listdir(self.file_path)) if file.endswith('.mat')]
        return self._get_n_signals(file_names=file_names, n_signals=n_signals)
    
    def _get_n_signals(self, file_names, n_signals):
...
#             return file_names
    ...
    def _load_mat_file(self, file_name):
        ...
        return sio.loadmat(os.path.join(self.file_path, file_name))['data'][:,0].astype('float')
