...
# Local imports
from higuchi_fractal_dimension import hfd
...
class FullWaveformFeatures:
    ...
    def calculate_stationary_wavelet_transform_features(self):
        ...
        for level in range(len(swt)):
            ...
            stationary_wavelet_transform_features['swt_d_' + str(level+1) + '_low_power_ratio'] =                 max_power_low / mean_power_low
            stationary_wavelet_transform_features['swt_d_' + str(level+1) + '_med_power_ratio'] =                 max_power_med / mean_power_med
            stationary_wavelet_transform_features['swt_d_' + str(level+1) + '_high_power_ratio'] =                 max_power_high / mean_power_high
            ...
            # Calculate max/mean power ratio
            stationary_wavelet_transform_features['swt_a_' + str(level+1) + '_low_power_ratio'] =                 max_power_low / mean_power_low
            stationary_wavelet_transform_features['swt_a_' + str(level+1) + '_med_power_ratio'] =                 max_power_med / mean_power_med
            stationary_wavelet_transform_features['swt_a_' + str(level+1) + '_high_power_ratio'] =                 max_power_high / mean_power_high
        ...
        for level in range(len(swt)):
            ...
            """Detail"""
            # Log-energy entropy
            stationary_wavelet_transform_features['swt_d_' + str(level+1) + '_energy_entropy'] =                 np.sum(np.log10(np.power(swt[level]['d'], 2)))

            # Higuchi_fractal
            stationary_wavelet_transform_features['swt_d_' + str(level+1) + '_higuchi_fractal'] =                 hfd(swt[level]['d'], k_max=10)

            """Approximation"""
            # Log-energy entropy
            stationary_wavelet_transform_features['swt_a_' + str(level+1) + '_energy_entropy'] =                 np.sum(np.log10(np.power(swt[level]['a'], 2)))

            # Higuchi_fractal
            stationary_wavelet_transform_features['swt_a_' + str(level+1) + '_higuchi_fractal'] =                 hfd(swt[level]['a'], k_max=10)
